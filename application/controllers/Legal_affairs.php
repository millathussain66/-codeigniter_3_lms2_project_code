...
class Legal_affairs extends CI_Controller {
	...
	function __construct()
    {
		...
		$this->load->model('Legal_affairs_model', '', TRUE);
	...
	function view ($menu_group,$menu_cat,$menu_link=null,$submenu=null)
	{
		...
		$data = array(
			...
			'req_type'=>$this->User_model->get_parameter_data('ref_case_type_legal_affairs','name','data_status=1'),
            'related_authority'=>$this->User_model->get_parameter_data('ref_related_authority','name','data_status=1'),
            'case_name'=>$this->User_model->get_parameter_data('ref_case_name','name','data_status = 1 AND req_type=1'),
			'dealing_officer'=>$this->User_model->get_parameter_data('users_info','name','data_status = 1 AND FIND_IN_SET(20,user_group_id)'),
	   		'per_page' => $this->config->item('per_pagess')
		   );
		if($submenu=='case_details'){
			$result =array();
			$data['result']=$result;
			$data['pages']='legal_affairs/pages/case_details_grid';
		}elseif($submenu=='case_status_update'){
			$data['pages']='legal_affairs/pages/case_status_update';
		}elseif($submenu=='billing'){
			$data['pages']='legal_affairs/pages/bill_grid';
		}else{
			$data['pages']='legal_affairs/pages/grid';
		}
	...
	function get_case_details_result()
	{
		...
		$result =$this->Legal_affairs_model->get_all_cases();
		$str_html="";
		if(!empty($result))
		{
			$sl=0;
	...
	function get_case_details_info()
	{
		...
		$str='';
		$expense = array();
		$suit_file_details=$this->Legal_affairs_model->get_case_details_info($this->input->post('id'));
    	$expense = $this->Legal_affairs_model->get_all_expense_by_case($this->input->post('id'),$suit_file_details->req_type);
        $status_history = $this->Legal_affairs_model->get_case_status_history($this->input->post('id'));
	...
	function make_xl()
	{
        ...
        if ($_POST['region']!='' && $_POST['region']!=0) {
            $str_where.= " AND s.region=".$this->db->escape($_POST['region']);
        }
        ...
        DATE_FORMAT(s.prev_date,'%d-%b-%y') AS prev_date,cs.name as case_sts_prev_dt,
        d.name as district,csn.name as case_sts_next_dt,IF(s.next_dt_sts=1,DATE_FORMAT(s.next_date,'%d/%m/%Y'),s.next_date) AS next_date,
        lr.name as legal_region,ls.name as loan_segment,s.remarks,co.total_cost as total_legal_cost
	        ...
	        $headings4 = array('Sl No','A/C Number','A/C Name',
                ...
                'Case Status On The Previous','Next Date','Case Status On Next Date',
                'District','Legal Region','Protfolio','Final Remarks','Legal Cost');//,strtotime($dealdate)));
	...
	function grid()
	{
		...
		$result = $this->Legal_affairs_model->get_grid_data($this->input->get('filterscount'), $this->input->get('sortdatafield'), $this->input->get('sortorder'),$pagesize, $start);
	...
	function bill_grid()
	{
		...
		$result = $this->Legal_affairs_model->get_grid_data_bill($this->input->get('filterscount'), $this->input->get('sortdatafield'), $this->input->get('sortorder'),$pagesize, $start);
	...
	function get_bill_info(){
		...
		$rows = $this->Legal_affairs_model->get_add_action_data($id);
	...
	function case_sts_grid()
	{
		...
		$result = $this->Legal_affairs_model->get_grid_data_case_sts($this->input->get('filterscount'), $this->input->get('sortdatafield'), $this->input->get('sortorder'),$pagesize, $start);
	...
	function get_filing_info()
	{
		...
		$suit_info =$this->Legal_affairs_model->get_suit_info($suit_id);
		if (!empty($suit_info)) {
			$Message='ok';
		}
		else{
	...
	function get_filing_info_edit()
	{
		$csrf_token=$this->security->get_csrf_hash();
		$id = $this->input->post('id');
		$suit_info =$this->Legal_affairs_model->get_add_action_data($id);
		if (!empty($suit_info)) {
			$Message='ok';
		}
		else{
		...
		if($suit_info->org_loan_ac!='' && $suit_info->org_loan_ac!=NULL)
		{
			$org_card_no = $this->Common_model->stringEncryption('decrypt',$suit_info->org_loan_ac);
		}
		else
		{
			$org_card_no ='';
	...
	function add_edit_action()
	{
		...
		{
			$id = $this->Legal_affairs_model->add_edit_action();
		}
		else{
		...
		if(count($text)<=0){
			$Message='OK';
			$row=$this->Legal_affairs_model->get_add_action_data($id);
		}else{
	...
	function delete_action()
	{
		...
		if($this->session->userdata['ast_user']['login_status'])
		{
			$id = $this->Legal_affairs_model->delete_action();
		}
		else{
			$text[]="Session out, login required";
		}
	...
	{
		$csrf_token=$this->security->get_csrf_hash();
		$sah=$this->Legal_affairs_model->get_r_history($this->input->post('id'),$this->input->post('life_cycle'));
        $jTableResult = array();
	...
	function suit_file_details()
	{
		...
		$str='';
		$expense = array();
		$suit_file_details=$this->Legal_affairs_model->get_suit_file_details($this->input->post('id'));
	    	if (!empty($suit_file_details)) 
	    	{
	    		...
	    		$str .='<table style="width: 100%;" id="preview_table">
					...
					<tbody id="details_body">
						...
						<tr>
							<td width="50%" align="left"><strong>Loan Segment (Portfolio):</strong>'.$suit_file_details->loan_segment.'</td>
							<td width="50%" align="left"><strong>Case Number:</strong>'.$suit_file_details->case_number.'</td>
						</tr>
						<tr>
							<td width="50%" align="left"><strong>Region:</strong>'.$suit_file_details->region.'</td>
						...
						<tr>
							...
							<td width="50%" align="left"><strong>Recovery AM:</strong>'.$suit_file_details->recovery_am.'</td>
						...
						<tr>
							<td width="50%" align="left"><strong>Police Station:</strong>'.$suit_file_details->police_station.'</td>
							<td width="50%" align="left"><strong>Related Authority:</strong>'.$suit_file_details->related_authority.'</td>
	...
	{
		//echo $id;exit;
		$result=$this->Legal_affairs_model->get_suit_file_details($id);
		$data = array( 	
				   'option' => '',
				   'result' => $result,
				   'id' => $id,
				   'pages'=> 'legal_affairs/pages/form_verify',
				   'edit_row' => $editrow			   
				   );
	...
	function add_edit_case_sts()
	{
		...
		if($this->session->userdata['ast_user']['login_status'])
		{
			$id = $this->Legal_affairs_model->add_edit_case_sts();
		}
		else{
			$text[]="Session out, login required";
		}
	...
	function get_add_info_case_sts()
	{
		...
		$details=array();
		$id = $this->input->post('id');
		$row_info =$this->Legal_affairs_model->get_add_action_data_suit($id);
		if (!empty($row_info)) {
			$Message='ok';
		}
		else{
		...
		$pre_case_sts = $this->Legal_affairs_model->get_pre_case_sts($id);
		$status_history = $this->Legal_affairs_model->get_case_status_history($id);
		$expense_activities = $this->User_model->get_parameter_data('ref_schedule_charges_legal_affairs','name','data_status = 1');
	...
	function get_edit_info_case_sts()
	{
		...
		$details=array();
		$id = $this->input->post('id');
		$expense_data = $this->Legal_affairs_model->get_expense_data($id);
		$result = $this->Legal_affairs_model->get_add_action_data_case_sts($id);
		$row_info =$this->Legal_affairs_model->get_add_action_data_suit($result->file_id);
		if (!empty($row_info)) {
			$Message='ok';
		}
		else{
		...
		$req_type = $row_info->req_type;
		$district = $row_info->district;
		$pre_case_sts = $this->Legal_affairs_model->get_pre_case_sts($result->file_id);
		$status_history = $this->Legal_affairs_model->get_case_status_history($result->file_id);
		$expense_activities = $this->User_model->get_parameter_data('ref_schedule_charges_legal_affairs','name','data_status = 1');
        $lawyer =$this->User_model->get_parameter_data('ref_lawyer','name','data_status = 1 AND district="'.$row_info->district.'"');
		$court =$this->User_model->get_parameter_data('ref_court','name','data_status = 1 AND district="'.$row_info->district.'"');
		$district_list =$this->User_model->get_parameter_data('ref_legal_district','name','data_status = 1');
	...
	function delete_action_case_sts()
	{
		...
		if($this->session->userdata['ast_user']['login_status'])
		{
			$id = $this->Legal_affairs_model->delete_action_case_sts();
		}
		else{
			$text[]="Session out, login required";
		}
		...
		if(count($text)<=0){
			...
			else{$row=$this->Legal_affairs_model->get_add_action_data_case_sts($id);}
		}else{
			for($i=0; $i<count($text); $i++)
			{
				if($i>0){$Message.=',';}
	...
	function details_case_sts()
	{
		...
		$str = '';
		$id = $this->input->post('id');
		$details = $this->Legal_affairs_model->get_details_case_sts($this->input->post('id'));
		$prev_str = '';
		$new_str = '';
		$expense = $this->Legal_affairs_model->get_expense_details($this->input->post('id'),$details->req_type);
		$status_history = $this->Legal_affairs_model->get_case_status_history($details->file_id);
	...
	function bulk_operation($operation=NULL)
	{
		...
		$data = array( 	
			   ...
			   'pages'=> 'legal_affairs/pages/bulk_operation',		   
			   );
	...
	function load_bulk_data()
	{
	    ...
	    $csrf_token=$this->security->get_csrf_hash();
	    $grid_data=$this->Legal_affairs_model->get_bulk_data();
	...
	function bulk_acktion()
	{
		...
		if($this->session->userdata['ast_user']['login_status'])
		{
			$id = $this->Legal_affairs_model->bulk_acktion();
		}
		else{
			$text[]="Session out, login required";
		}
