...
class Cma_rt_model extends CI_Model {
	...
	function get_xl_daily_report_data(){
		$ToDate = implode('-',array_reverse(explode('/',trim((string)$this->input->post('ToDate'))))); 
		$FromDate = implode('-',array_reverse(explode('/',trim((string)$this->input->post('FromDate'))))); 
		$appFromDate = implode('-',array_reverse(explode('/',trim((string)$this->input->post('appFromDate'))))); 
		$appToDate = implode('-',array_reverse(explode('/',trim((string)$this->input->post('appToDate'))))); 
		$region_id = trim((string)$this->input->post('region_id'));
		$status_id = trim((string)$this->input->post('status_id'));
		$territory = trim((string)$this->input->post('territory'));
		$district = trim((string)$this->input->post('district'));
		...
		$this->db
			->select("j0.*,auc.name as pre_auc_sts,a.auction_sts,j0.cma_sts as sts,j15.name as cma_sts,CONCAT(j13.name,' (',j13.code,')') as branch_sol,
				j1.name as region_name,j2.name as territory_name,j3.name as district_name,
				j4.name as unit_office_name,r2.name as legal_region_name,
				j6.name as subject_name,j7.name as loan_segment,j8.name as e_by,
			...
			foreach($query as $i=>$product) {
				...
				}else if($product['proposed_type']=='Card'){


					   $this->db->select("f.id,DATE_FORMAT(f.card_issue_dt,'%d-%b-%Y') as card_issue_dt,
						...
						$this->db->from('sub_card_data as f');
						$this->db->where('f.module_id', $product['id']);

			   		$card_query = $this->db->get()->result_array();
			   		$query[$i]['facility_loan'] = $card_query;
