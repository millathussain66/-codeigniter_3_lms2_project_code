...
/* .center {
  ...
  padding: 10px;
    ...
    var paper = [<? $i=1; foreach($paper as $row){ if($i!=1){echo ',';} echo '{value:"'.$row->id.'", label:"'.$row->name.'"}'; $i++;}?>];
		...
		jQuery("#paper").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select Paper", source: paper, width: 250, height: 25});
		jQuery("#paper_vendor").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select Paper Vendor", source: paper_vendor, width: 250, height: 25});
		...
		jQuery("#auction_time").jqxDateTimeInput({ width: '250px', height: '25px', formatString: 'HH:mm:ss', showTimeButton: true, showCalendarButton: false});
		jQuery('#paper_form').jqxValidator({
			rules: [
				{ input: '#paper', message: 'required!', action: 'blur,change', rule: function (input) {					
					if(input.val() != '')
					{
						var item = jQuery("#paper").jqxComboBox('getSelectedItem');
						if(item != null){jQuery("input[name='paper']").val(item.value);}
						return true;				
					}
					else
					{
				...
				{ input: '#paper_date', message: 'required!', action: 'keyup, blur, change', rule: function(input,commit){
				...
				{ input: '#paper_date', message: 'Invalid', action: 'keyup, blur, change', rule: function (input, commit){
					if (!input.val()) {
						return true;
					}
				...
				{ input: '#paper_date', message: 'Old Date Not allowed !', action: 'change', rule: function(input,commit){  
	                	if(input.val()!="") { return validateDate(input.val()); } else{return true;}
	              }
	            },
				{ input: '#auction_date', message: 'required!', action: 'keyup, blur, change', rule: function(input,commit){
				...
				{ input: '#auction_date', message: 'Invalid', action: 'keyup, blur, change', rule: function (input, commit){
					if (!input.val()) {
						return true;
					}
					if(validate_date(input.val()))
				...
				{ input: '#auction_date', message: 'Old Date Not allowed !', action: 'change', rule: function(input,commit){  
	                	if(input.val()!="") { return validateDate(input.val()); } else{return true;}
	              }
	            },
				{ input: '#auction_time', message: 'required!', action: 'keyup, blur, change', rule: function(input,commit){
				...
				{ input: '#auction_address', message: 'required!', action: 'keyup, blur', rule: function(input,commit){
				...
				{ input: '#auction_address', message: 'more than 250 characters', action: 'keyup, blur, change', rule: function (input, commit)
				 {
				 	if(input.val() != '')
					{
	        						...
	        						<td width="50%">
					        			<table style="width: 100%;" border="0">
					        				<tr>
												...
												<td width="60%" style="border:0px"><div id="paper_vendor" name="paper_vendor" style="padding-left: 3px"></div></td>
					        				</tr>
					        				<tr>
												<td style="border:0px"><strong>Paper Name</strong> <span style="color:#FF0000">*</span></td>
												<td style="border:0px"><div id="paper" name="paper" style="padding-left: 3px"></div></td>
											</tr>
											<tr>
												<td style="border:0px"><strong>Paper Date</strong> <span style="color:#FF0000">*</span></td>
												<td style="border:0px"><input type="text" style="width:250px" name="paper_date"  placeholder="dd/mm/yyyy"  id="paper_date" value="<?=isset($result->paper_date)?date_format(date_create($result->paper_date),"d/m/Y"):''?>" ><script type="text/javascript" charset="utf-8">datePicker ("paper_date");</script></td>
											</tr>
											<tr>
												<td style="border:0px"><strong>Auction Date</strong> <span style="color:#FF0000">*</span></td>
												<td style="border:0px"><input type="text" style="width:250px" name="auction_date" placeholder="dd/mm/yyyy"  id="auction_date" value="<?=isset($result->auction_date)?date_format(date_create($result->auction_date),"d/m/Y"):''?>" ><script type="text/javascript" charset="utf-8">datePicker ("auction_date");</script></td>
											</tr>
					        			</table>
					        		</td>
					        		<td width="50%">
