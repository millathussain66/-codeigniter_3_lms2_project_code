...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		jQuery("#legal_region").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Legal Region", source: legal_region, width: '98%', height: 25});
		...
		jQuery('#legal_district').bind('change', function (event) {
			change_dropdown('legal_district_case_deal_officer');		
		});
	...
	function change_dropdown(operation,edit=null)
	{
		...
		if (edit==null && operation!='legal_district_case_deal_officer') 
		{
        ...
        success: function(response){
        	var json = jQuery.parseJSON(response);
					...
					if (operation=='legal_district_case_deal_officer') 
					{
						...
						jQuery("#case_deal_officer").jqxComboBox({theme: theme, autoDropDownHeight: false, promptText: "Case Deal Officer", source: case_deal_officer, width: '98%', height: 25});
					}
	...
	{
		<?php if (!(check_group('10') || check_group('11') || check_group('18'))): ?>
			if (jQuery('#legal_region').val() == '') {
	    		alert('Region required!');
	    		jQuery('#legal_region').focus();
	    		return false;
	    	}
		<?php endif ?>
		...
		jQuery.ajax({
				type: "POST",
				cache: false,
				url: "<?=base_url()?>index.php/authorization_ho/load_bulk_data/",
				data : postdata,
				datatype: "json",
				success: function(response){
						var json = jQuery.parseJSON(response);
						...
						if (jQuery("#operation").val()=='sta'  && jQuery('#event_counter').val()!=0)
						{
							...
							jQuery("#lawyer").jqxComboBox({theme: theme, autoDropDownHeight: false, promptText: "Select lawyer", source: lawyer, width: '97%', height: 21});

							var holm = [];
							jQuery.each(json['checker'],function(key,obj){
								holm.push({ value: obj.id, label: obj.name });
							});
	...
	{							
		...
		genereate_download_link();								
	}
	function genereate_download_link()
	{
		var number=jQuery("#event_counter").val();
		var ids = 'download';
		for(var i=1; i<=number; i++){														
			...
			{
				ids = ids+'_'+jQuery("#id_"+i).val();
			}						
		}
		var strLink2 = '<?=base_url()?>index.php/authorization_ho/bulk_download_pdf/'+ids;
	...
	function details(id,operation,type,event_id)
	{
		...
		jQuery.ajax({
			...
			data : {[csrfName]: csrfHash,deleteEventId: id,type: type,EventId: event_id
			},
			datatype: "json",
			success: function(response){
			//alert(response);
	...
	function call_ajax_submit()
	{
		...
		jQuery.ajax({
			type: "POST",
			cache: false,
			url: '<?=base_url()?>index.php/authorization_ho/bulk_acktion/',
			data : postData,
			datatype: "json",
			success: function(response){
		...
		<form class="form" id="form" name="form"  method="post" action="<?=base_url()?>legal_notice_ho/upload_file" enctype="multipart/form-data">
			...
			<div style="margin-left:10px;padding: 0.5%;width:98%;height:65px; border:1px solid #c0c0c0;font-family: Calibri;font-size: 14px">
					<table id="deal_body" style="display:block;width:100%">
						<tr>
							...
							<td style="width:8%"><input id="req_dt_from" name="req_dt_from" style="width:40%" /><script type="text/javascript">datePicker("req_dt_from");</script>
							...
							<td style="text-align:right;width:5%"><strong>Legal Region&nbsp;&nbsp;</strong> </td>
							<td style="width:5%"><div id="legal_region" tabindex="28" name="legal_region" style="padding-left: 3px"></div></td>
						...
						<tr>
							<td style="text-align:right;width:5%"><strong>Authorization Date From&nbsp;&nbsp;</strong> </td>
							<td style="width:8%"><input id="auth_dt_from" name="auth_dt_from" style="width:40%" /><script type="text/javascript">datePicker("auth_dt_from");</script>
							<strong>To</strong> <input id="auth_dt_to" name="auth_dt_to" style="width:40%" /><script type="text/javascript" >datePicker("auth_dt_to");</script>
							</td>
			...
			<table id="submit_button_table"  align="center" style="font-size:15px !important;margin-bottom:20px !important;display:none" border="0" cellspacing="0" cellpadding="2">
			  <tr align="center"><td style="border:0px; vertical-align:top;">		
				...
				&nbsp;&nbsp;&nbsp;
				<?php if ($operation!='download'): ?>
					<input type="button" value="Submit" id="sendButton" onclick="delete_action()" class="btn-small btn-small-normal enabledElem" style="display: inline;cursor:pointer;font-size:16px;" />&nbsp;&nbsp;
					<span id="loading" style="display:none">Please wait... <img src="<?=base_url()?>images/loader.gif" align="bottom"></span><br/>
				<?php else: ?>
					<a id="pdf" style="text-align:center;cursor:pointer;" href="" target="_blank" ><img align="center" src="<?=base_url()?>images/pdf_icon.gif" style="width:30px"></a>&nbsp;&nbsp;
				<?php endif ?>
				<br/>
				</td>
			  </tr>
