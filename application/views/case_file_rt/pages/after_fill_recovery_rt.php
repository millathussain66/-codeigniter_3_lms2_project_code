...
<?php if ($operation=='after_fill_recovery_rt'): ?>
    <?php 
        $report_category_segment = array();

        $report_category_segment['1'] ='Account Wise';
        $report_category_segment['2']='Dealing Officer Wise';
        $report_category_segment['3']='Case Type Wise';



        $report_category=array();

        $report_category['1'] ='Region Wise';
        $report_category['5'] ='District Wise';
        $report_category['6'] ='Territory Wise';
        $report_category['3'] ='Protfolio Wise';
       ...
       $reporting_period=array();
        ...
        $present_status['1'] = 'After Case Filing (1st Date)';
        $present_status['2'] = 'After (2nd Filing Date)';
        $present_status['3'] = 'After Balance Date';
    ...
    <script type="text/javascript">
        ...
        var report_category_segment = [<? $i=1; foreach($report_category_segment as $key=>$row){ if($i!=1){echo ',';} echo '{value:"'.$key.'", label:"'.$row.'"}'; $i++;}?>];
      

        var report_category = [<? $i=1; foreach($report_category as $key=>$row){ if($i!=1){echo ',';} echo '{value:"'.$key.'", label:"'.$row.'"}'; $i++;}?>];
       
        var reporting_period = [<? $i=1; foreach($reporting_period as $key=>$row){ if($i!=1){echo ',';} echo '{value:"'.$key.'", label:"'.$row.'"}'; $i++;}?>];

        var reporting_time = [<? $i=1; foreach($reporting_time as $key=>$row){ if($i!=1){echo ',';} echo '{value:"'.$key.'", label:"'.$row.'"}'; $i++;}?>];
       
        var present_status = [<? $i=1; foreach($present_status as $key=>$row){ if($i!=1){echo ',';} echo '{value:"'.$key.'", label:"'.$row.'"}'; $i++;}?>];
        ...
        jQuery(document).ready(function () {
            jQuery("#from_year").jqxDropDownList({theme: theme,autoDropDownHeight: false, dropDownHeight: 100,promptText: "From",source: year, width: 65, height: 25});
            jQuery("#to_year").jqxDropDownList({theme: theme,autoDropDownHeight: false, dropDownHeight: 100,promptText: "To",source: year, width: 65, height: 25});
           

            jQuery("#from_year_cat").jqxDropDownList({theme: theme,autoDropDownHeight: false, dropDownHeight: 100,promptText: "From",source: year, width: 65, height: 25});
            jQuery("#to_year_cat").jqxDropDownList({theme: theme,autoDropDownHeight: false, dropDownHeight: 100,promptText: "To",source: year, width: 65, height: 25});
           


            jQuery("#report_category_segment").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Report Type", source: report_category_segment, width: 150, height: 25});
           
            jQuery("#report_category").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Report Catagory", source: report_category, width: 150, height: 25});
            jQuery("#reporting_time").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Reporting Time", source: reporting_time, width: 150, height: 25});


            jQuery("#reporting_period").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Reporting Period", source: reporting_period, width: 150, height: 25});
          


            jQuery("#present_status").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Present Status", source: present_status, width: 150, height: 25});
    ...
    function change_report_category_dropdown () {
      ...
      var id='';
        ...
        if(item!=null)
        {
            ...
            {
                table_name='users_info';      jQuery("#from_year_cat").hide();
                jQuery("#to_year_cat").hide();
        ...
        async:false,
        type: "post",
        data: { [csrfName]: csrfHash,table_name:table_name},
        datatype: "json",
                ...
                var json = jQuery.parseJSON(response);
                        ...
                        if(table_name=='users_info')
                        {
                            jQuery.each(json['row_info'],function(key,obj){
                                drop_down_data.push({ value: obj.id, label: obj.name+'('+obj.pin+')' });
                            });
                        }
                        else if(table_name=='ref_loan_segment')
                        {
                            jQuery.each(json['row_info'],function(key,obj){
                                drop_down_data.push({ value: obj.code, label: obj.name });
                            });
                        }
                        else
                        {
                            jQuery.each(json['row_info'],function(key,obj){
                                drop_down_data.push({ value: obj.id, label: obj.name });
                            });
                        }
                        if(item.value==2)
                        {
                            jQuery("#report_category_dropdown_multi").jqxDropDownList({theme: theme,checkboxes: true, autoDropDownHeight: false, promptText: "Select",filterable: true,searchMode: 'containsignorecase', source: drop_down_data, width: 150, height: 25});
                            jQuery("#report_category_dropdown_multi").show();
                            jQuery("#report_category_dropdown").hide();
                        }
                        else
                        {
                            jQuery("#report_category_dropdown").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select", source: drop_down_data, width: 150, height: 25});
    ...
    function change_report_category_sagment_dropdown()
       {
        ...
        success: function(response){
                var json = jQuery.parseJSON(response);
                        ...
                        if(item.value==2)
                        {
                            jQuery("#report_category_sefment_dropdown_multi").jqxDropDownList({theme: theme,checkboxes: true, autoDropDownHeight: false, promptText: "Select",filterable: true,searchMode: 'containsignorecase', source: drop_down_data, width: 150, height: 25});
                            jQuery("#report_category_sefment_dropdown_multi").show();
                            jQuery("#report_category_segment_dropdown").hide();
                        }
                        else
                        {
                            jQuery("#report_category_segment_dropdown").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select", source: drop_down_data, width: 150, height: 25});
    ...
    {
        var postdata = jQuery('#after_fill_recovery_form').serialize();
        jQuery("#report_result").html('');
        jQuery("#report_generate").hide();
        jQuery("#grid_loading").show();

        jQuery.ajax({
            url: '<?php echo base_url(); ?>index.php/case_file_rt/after_fill_recovery_rt_result',
            async:false,
            type: "post",
            data : postdata,
            datatype: "json",
                    ...
                    <td valign="top" id="demos" class='rc-all content'>
                        ...
                        <div>
                            <div style="overflow: hidden;">
                                ...
                                <form class="form" name="after_fill_recovery_form" id="after_fill_recovery_form" method="post" action="<?= base_url()?>Case_file_rt/after_fill_recovery_xl" enctype="multipart/form-data">
                                            ...
                                            <td style="vertical-align: top;">
                                              ...
                                              <td style="vertical-align: top;">
                                               ...
                                               <td style="vertical-align: top;">
                                                ...
                                                <input type="text" name="filling_dt_from" placeholder="From Date" style="display: none;width:65px;" id="filling_dt_from" value="" ><script type="text/javascript" charset="utf-8">datePicker ("filling_dt_from");</script>
                                                <input type="text" name="filling_dt_to" placeholder="To Date" style="display: none;width:71px;" id="filling_dt_to" value="" ><script type="text/javascript" charset="utf-8">datePicker ("filling_dt_to");</script>
                                               </td> 
