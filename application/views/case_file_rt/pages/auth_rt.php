...
<?php if ($operation=='auth_rt'): ?>
    ...
    <script type="text/javascript">
        ...
        var req_type = [<? $i=1; foreach($authorization_type as $row){ if($i!=1){echo ',';} echo '{value:"'.$row->id.'", label:"'.$row->name.'"}'; $i++;}?>];
        ...
        jQuery(document).ready(function () {
            ...
            jQuery("#req_type").jqxDropDownList({theme: theme,checkboxes: true, autoDropDownHeight: false, promptText: "Type Of Authorizaion",filterable: true,searchMode: 'containsignorecase', source: req_type, width: 150, height: 25});
    ...
    {
        var postdata = jQuery('#auth_rt_form').serialize();
        jQuery("#report_result").html('');
        jQuery("#report_generate").hide();
        jQuery("#grid_loading").show();

        jQuery.ajax({
            url: '<?php echo base_url(); ?>index.php/case_file_rt/auth_rt_form_result',
            async:false,
            type: "post",
            data : postdata,
            datatype: "json",
                    ...
                    <td valign="top" id="demos" class='rc-all content'>
                        ...
                        <div>
                            <div style="overflow: hidden;">
                                ...
                                <form class="form" name="auth_rt_form" id="auth_rt_form" method="post" action="<?= base_url()?>Case_file_rt/auth_rt_xl" enctype="multipart/form-data">
