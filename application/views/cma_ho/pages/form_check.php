...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		jQuery('#decline_button').jqxButton({template:"warning", width: 120, height: 40, theme: theme });		
	...
	function call_ajax_submit()
	{
    	...
    	if (jQuery('#checker_to_notify').val()=='' && jQuery('#type').val()=='sendtochecker') 
		{
		...
		{
			var postData = $('legal_notice_form').toQueryString()+"&"+csrfName+"="+csrfHash;
		}
		else
		{
...
<body style="font-family:calibri">    
			...
			<td style="width:30%">
				<select name="checker_to_notify" id="checker_to_notify">
		      		...
		      		<?php foreach ($checker_info as $checker): ?>
		      			<?php $count++; ?>
		          		<?php if ($checker->pin=='4402'): ?>
		          			<option value="<?=$checker->id?>" <?php echo "selected"; ?> ><?=$checker->name.' ('.$checker->pin.')'?></option>
		          		<?php else: ?>
		          			<option value="<?=$checker->id?>"><?=$checker->name.' ('.$checker->pin.')'?></option>
		          		<?php endif ?>
		      		<?php endforeach; ?>
		      	</select>
	      	</td>
		...
		<table  align="center" style="font-size:15px !important;margin-bottom:20px !important" border="0" cellspacing="0" cellpadding="2">
			  <tr align="center"><td style="border:0px; vertical-align:top;">		
				...
				&nbsp;&nbsp;&nbsp;
					<br/>
					&nbsp;&nbsp;&nbsp;
					<input type="button" value="Send to(HOLM)" id="sendButton" onclick="delete_action('sendtochecker')" class="btn-small btn-small-normal enabledElem" style="display: inline;cursor:pointer;font-size:16px;" />&nbsp;&nbsp;
	...
	<div id="sendToCheckerMessageDialogContent"  style="display:none;">
	    ...
	    <form method="POST" name="sendToCheckerMessageform" id="sendToCheckerMessageform"  style="margin:0px;">
	       <input name="type" id="type" value="return" type="hidden">
