...
<script type="text/javascript">
	...
	jQuery(document).ready(function() {
		...
		<? if ($add_edit == 'edit') { ?>
			...
			<? if ($result->sub_type == '3') { ?>
				jQuery('#spfm').show();
			<? } ?>
			<? if ($result->sec_sts == '1' && $result->req_type != 1) { ?>
		...
		jQuery('#sec_sts').bind('change', function(event) {
			...
			var item2 = jQuery("#req_type").jqxComboBox('getSelectedItem');
			if (item2 != null && item!=null && item2.value != 1 && item.value == 1 ) {
				jQuery('#cma_doc_div').show();
				document.getElementById('auction_div').style.display = '';
			} else {
		...
		jQuery('#req_type').bind('change', function(event) {
			...
			if (item!=null && item.value != 2 && item.value != 5) {
	...
	function clear_desable_facility_checkbox() {
		...
		var item2 = jQuery("#proposed_type").jqxComboBox('getSelectedItem');
		var loan_ac = jQuery("#loan_ac").val();
		if (item != null && item.value == 1 && item2 != null && item2.value == 'Loan') //when request name is NI-ACT other facility will be desabe for select as per mail request by prince(13-12-2022)
		{
			var number = jQuery("#facility_info_counter").val();
			for (var i = 1; i <= number; i++) {
				jQuery("#facility_info_delete_" + i).val(1);
	...
	function owner_validation() {
		...
		//Doc File Validation
		var item = jQuery("#sec_sts").jqxComboBox('getSelectedItem');
		var req_item = jQuery("#req_type").jqxComboBox('getSelectedItem');
		//for loan owner validation
		if (item!=null && item.value == 1 && req_item != null && req_item.value != 1) {
			//alert('ss');
						...
						<tr>
								...
								<table style="width: 100%;">
									...
									<tr>
										...
										<td width="60%">
											...
											<?php else : ?>
												<input name="loan_ac" tabindex="3" type="text" class="" maxlength="16" size="16" style="width:250px" id="loan_ac" value="<?= isset($result->loan_ac) ? $result->loan_ac : '' ?>" />
											<?php endif ?>
										</td>
									</tr>
									...
									<tr>
										<td width="40%" style="font-weight: bold;">CIF<span style="color:green">*</span> </td>
									...
									<tr>
										<td width="40%" style="font-weight: bold;">Chq Expiry Date<span style="color:green">*</span> </td>
							...
							<td width="50%">
								<table style="width: 100%;">
									...
									<tr>
										<td width="40%" style="font-weight: bold;">Current DPD<span style="color:green">*</span> </td>
									...
									<tr>
										<td width="40%" style="font-weight: bold;">Logic for ARA Case<span style="color:green">*</span> </td>
										<td width="60%">
						...
						<table border="1" id="gurantor_table" style="border-color:#c0c0c0;width:130%;margin:20px">
							<thead>
								<tr>
									...
									<td width="3%" style="font-weight: bold;text-align:center">Int. Rate<span style="color:red">*</span></td>
									...
									<td width="5%" style="font-weight: bold;text-align:center">CL(BBL)<span style="color:red">*</span></td>
...
<script type="text/javascript">
		...
		},
		{
			...
			rule: function(input, commit) {
				if (jQuery("#call_up_serv_dt").val() == '') {
					var item3 = jQuery("#proposed_type").jqxComboBox('getSelectedItem');
					if(item3!=null && item3.value=='Loan')
					{
						var item = jQuery("#req_type").jqxComboBox('getSelectedItem');
						if (item!=null && item.value != 1) {
							jQuery("#call_up_serv_dt").focus();
							return false;
						} else if (item!=null && item.value == 1 && dateCheck() == false) {
							return true;
						} else {
							return false;
						}
					}
					else if(item3!=null && item3.value=='Card')
					{
						var cl_bb = jQuery("#cl_bb_card").jqxComboBox('getSelectedItem');
						var cl_bbl = jQuery("#cl_bbl_card").jqxComboBox('getSelectedItem');
						if(cl_bb!=null && cl_bbl!=null)
						{
							var item = jQuery("#req_type").jqxComboBox('getSelectedItem');
							if(cl_bb.value=='w-off' || cl_bbl.value=='w-off')
							{
								return true;
							}
							else if (item!=null && item.value != 1) {
								jQuery("#call_up_serv_dt").focus();
								return false;
							} else if (item!=null && item.value == 1 && dateCheck() == false) {
								return true;
		...
		},
		{
			...
			rule: function(input) {
				if (input.val() != '') {
					...
					var item3 = jQuery("#req_type").jqxComboBox('getSelectedItem');
					if (item3 != null && item3.value == 2) {
	...
	jQuery("#in_req_button").click(function() {
		...
		var validationResult = function(isValid) {
			if (isValid && owner_validation() == true && facility_validation() == true && check_duplicate_ara_case() == true) {
