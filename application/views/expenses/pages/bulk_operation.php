...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		<?php if ($operation=='lawyer_apv' || $operation=='court_fee_apv'): ?>
		jQuery("#lawyer").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select Lawyer", source: lawyer, width: '98%', height: 25});
		...
		jQuery('#legal_district,#case_deal_officer,#legal_region').focusout(function() {
            commbobox_check(jQuery(this).attr('id'));
        });
        ...
        success: function(response){
        	var json = jQuery.parseJSON(response);
					...
					if (operation=='legal_district_lawyer') 
					{
						...
						jQuery("#lawyer").jqxComboBox({theme: theme, autoDropDownHeight: false, promptText: "Select Lawyer", source: lawyer, width: '98%', height: 25});
					}
	...
	function search_data()
	{
		...
		jQuery.ajax({
				type: "POST",
				cache: false,
				url: "<?=base_url()?>index.php/expenses/load_bulk_data/",
				data : postdata,
				datatype: "json",
				success: function(response){
						var json = jQuery.parseJSON(response);
	...
	function call_ajax_submit()
	{
		...
		jQuery.ajax({
			type: "POST",
			cache: false,
			url: '<?=base_url()?>index.php/expenses/bulk_acktion/',
			data : postData,
			datatype: "json",
			success: function(response){
...
<body style="font-family:calibri">    
	...
	<div align="left">	
		<form class="form" id="form" name="form"  method="post" action="<?=base_url()?>legal_notice_ho/upload_file" enctype="multipart/form-data">
			...
			<input type="hidden" id="module_name" name="module_name" value="<?=$operation;?>" />
			...
			<div style="margin-left:10px;padding: 0.5%;width:98%;height:65px; border:1px solid #c0c0c0;font-family: Calibri;font-size: 14px">
					<table id="deal_body" style="display:block;width:100%">
						...
						<?php if ($operation=='lawyer_apv' || $operation=='court_fee_apv'): ?>
						<tr>
							<td style="text-align:left;width:5%"><strong>Lawyer&nbsp;&nbsp;</strong> </td>
							<td style="width:5%"><div id="lawyer" tabindex="28" name="lawyer" style="padding-left: 3px"></div></td>
						</tr>
						<?php endif ?>
