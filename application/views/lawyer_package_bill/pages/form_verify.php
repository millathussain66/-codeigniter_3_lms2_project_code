...
<script type="text/javascript">
	...
	function delete_action(type) {
		...
		if (type=='verify') 
		{
			...
			jQuery("#success_message").val('Verifyed');
		}
		if (type=='verify_enhance') 
		{
			jQuery("#reason_messge_body").hide();
			jQuery("#type").val(type);
			jQuery("#message").html('Verify');
	...
	{
		if (jQuery('#reason').val() == '' && jQuery('#type').val()!='verify' && jQuery('#type').val()!='verify_enhance') {
	...
	<div align="left">	
		<form class="form" id="form" name="form"  method="post" action="#" enctype="multipart/form-data">
			...
			<table class="custtable" border="0" style="margin-left:10px;margin-top:10px;width:99%; font-size:15px;border-collapse:collapse">
				...
				<tr>
					<td style="width:12%"><strong>Package Type</strong></td>
					<td style="width:30%"><?=$result->package_type_name;?></td>
				...
				<tr>
					...
					<td style="width:12%"><strong>Package Amount</strong></td>
					<td style="width:30%"><?=$result->package_amount;?></td>
				</tr>
				<tr>
					<td style="width:12%"><strong>Loan Segment</strong></td>
				...
				<tr>
					...
					<td style="width:30%">
						<?php if ($result->package_approval_file!=''): ?>
							<img id="file_preview" onclick="popup('<?=base_url()?>package_approval_file/<?=$result->package_approval_file?>')" style=" cursor:pointer;text-align:center" height="18" src="<?=base_url()?>old_assets/images/print-preview.png">
						<?php endif ?>
					</td>
				...
				<?php if ($result->v_sts==100): ?>
				<tr>
					<td style="width:12%"><strong>New Package Amount</strong></td>
					<td style="width:30%"><?=$result->inhanced_amount;?></td>
					<td style="width:12%"><strong>Reason </strong></td>
					<td style="width:30%"><?=$result->enhance_reason;?></td>
				</tr>
				<tr>
					<td style="width:12%"><strong>Enhance File</strong></td>
					<td style="width:30%">
						<?php if ($result->enhance_file!=''): ?>
							<img id="file_preview" onclick="popup('<?=base_url()?>package_approval_file/<?=$result->enhance_file?>')" style=" cursor:pointer;text-align:center" height="18" src="<?=base_url()?>old_assets/images/print-preview.png">
						<?php endif ?>
					</td>
					<td style="width:12%"><strong>Enhance By </strong></td>
					<td style="width:30%"><?=$result->enhance_by;?></td>
				</tr>
				<?php endif ?>
			</table>
			<table  align="center" style="font-size:15px !important;margin-bottom:20px !important" border="0" cellspacing="0" cellpadding="2">
			  <tr align="center"><td style="border:0px; vertical-align:top;">		
				...
				&nbsp;&nbsp;&nbsp;
				<?php if ($result->v_sts==100): ?>
					<input type="button" value="verify" id="sendButton" onclick="delete_action('verify_enhance')" class="btn-small btn-small-normal enabledElem" style="display: inline;cursor:pointer;font-size:16px;" />&nbsp;&nbsp;
					<input type="button" value="Decline" id="decline_button" onclick="delete_action('reject_enhance')" class="btn-small btn-small-normal enabledElem" style="display: inline;;cursor:pointer;font-size:16px;" />&nbsp;&nbsp;
				<?php else: ?>
						<input type="button" value="verify" id="sendButton" onclick="delete_action('verify')" class="btn-small btn-small-normal enabledElem" style="display: inline;cursor:pointer;font-size:16px;" />&nbsp;&nbsp;
