...
<div id="container">
		...
		<? } ?>
	      // prepare the data
		   ...
		   var columnCheckBox = null;
						...
						{ text: 'Reason', datafield: 'return_reason',cellclassname:cellclassname,hidden:true,  editable: false,  width: '4%' },
 						<? if(DELETE==1){?>
					  	...
					  	cellsrenderer: function (row) {
							editrow = row;
							var dataRecord = jQuery("#jqxgrid").jqxGrid('getrowdata', editrow);
							if((<?=$this->session->userdata['ast_user']['user_id']?>==dataRecord.e_by_id || <?=$this->session->userdata['ast_user']['user_system_admin_sts']?>=='2' || check_group_javascript(11) || check_group_javascript(10)) && (dataRecord.legal_notice_sts == 1 || dataRecord.legal_notice_sts == 4 || dataRecord.legal_notice_sts == 9)){
			...
			jQuery('#region,#loan_segment,#territory,#district,#unit_office,#status').focusout(function() {
	...
	function details(id,operation,indx,territory=null,region=null)
	{
		...
		{
			jQuery("#header_title").html('1st Legal Notice Details');
		...
		{
			jQuery('#comments').val('');
			jQuery("#header_title").html('Delete 1st Legal Notice');
			jQuery('#sendtochecker_row').hide();
		...
		{
			jQuery("#header_title").html('Send to Checker 1st Legal Notice');
  ...
  <div id="container" style="">
	<div id="body"  >
		<div  style="display:block; min-height:340px; height:auto">
		    ...
		    href="<?=base_url()?>index.php/legal_notice/from/add" title=""><input type="button" class="buttonStyle" style="background-color:#c2c2bc;font-size:13px;border-radius: 3px !important;height:40px;width:180px;"  value="Initiate 1st Legal Notice" id="sendButton" /></a>
		    <? }?> &nbsp;&nbsp;
		     <? if(BULKRECOMMEND==1){?>
			<a style="text-decoration:none" onclick="javascript:EOL.messageBoard.open(this.href, (jQuery(window).width()-70), jQuery(window).height(), 'yes'); return false;"
		    href="<?=base_url()?>index.php/legal_notice/bulk_operation/recommend" title=""><input type="button" class="buttonStyle" style="background-color:#c2c2bc;font-size:13px;border-radius: 3px !important;height:40px;width:140px;"  value="BULK Recommend" id="sendButton" /></a>
		    <? }?> &nbsp;&nbsp;
