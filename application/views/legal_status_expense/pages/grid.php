...
/* .center {
  ...
  padding: 10px;
    ...
    <script type="text/javascript">
            ...
            var req_type_array = [<? $i=1; foreach($req_type as $row){ if($i!=1){echo ',';} echo '{value:"'.$row->id.'", label:"'.$row->name.'"}'; $i++;}?>];
            ...
            jQuery("#req_type").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false,dropDownHeight: 100, promptText: "Type Of Case", source: req_type_array, width: 120, height: 30});
            jQuery("#req_type_grid").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false,dropDownHeight: 100, promptText: "Type Of Case", source: req_type_array, width: 120, height: 30});
            var theme = 'classic';
            jQuery("#next_dt_sts").jqxCheckBox({width: 22, theme: theme ,checked:true });
            jQuery("#not_available_sts").jqxCheckBox({width: 22, theme: theme ,checked:false });
           ...
           jQuery('#proposed_type,#proposed_type_grid,#req_type,#req_type_grid').focusout(function() {
           ...
           jQuery("#next_date_sts_grid").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false,dropDownHeight:100, promptText: "Case Status", source: case_sts, width: 250, height: 25});
			...
			jQuery('#case_sts,#next_date_purpose,#final_remarks,#next_date_sts_grid').focusout(function() {
	            commbobox_check(jQuery(this).attr('id'));
	        });
           ...
           change_caption_grid();
            ...
            jQuery('#case_sts').bind('change', function (event) {
                ...
                if (item!=null)
                {
                    ...
                    //checking the close account flag for ara judgement
                    if(item.value=='15' && req_type=='2')
                    {
                        set_final_remarks_by_close_flag();
                    }
                    else
                    {
                ...
                var source =
                {
                   ...
                   datafields: [
                    ...
                    var dataadapter = new jQuery.jqx.dataAdapter(source, {
                            ...
                            var req_type_grid = jQuery.trim(jQuery('#req_type_grid').val());
                    ...
                    jQuery("#jqxgrid").jqxGrid(
                    {
						...
						<? if(DELETE==1){?>
						  { text: 'D', datafield: 'delete', editable: false,align:'center', sortable: false, menu: false, width: 35,
	                      	cellsrenderer: function(row) {
                                ...
                                else if(<?=DELETE?>=='1' && (check_group_javascript(11) || check_group_javascript(10)) && dataRecord.sts==51)
                                {
                                    return '<div style="text-align:center;margin-top: 5px;  cursor:pointer" onclick="details('+dataRecord.id+',\'ho_delete\')" ><img align="center" src="<?=base_url()?>images/delete-New.png"></div>';
                                }
							 	else {
						...
						<? if(EDIT==1){?>
						  { text: 'E', datafield: 'edit', editable: false,align:'center', sortable: false, menu: false, width: 35,
	                      	cellsrenderer: function(row) {
                                ...
                                else if(<?=EDIT?>=='1' && (check_group_javascript(11) || check_group_javascript(10)))
                                {
                                    return '<div style="text-align:center;margin-top: 5px;  cursor:pointer" onclick="edit('+dataRecord.id+','+editrow+',\'1\')" ><img align="center" src="<?=base_url()?>images/edit-new.png"></div>';
                                }
								else
								{
									return '<div style=" margin-top: 5px; cursor:pointer;text-align:center"></div>';
                        ...
                        { text: 'Case number', datafield: 'case_number',editable: false, width: '15%', align:'left',cellsalign:'left'},
    ...
    function set_final_remarks_by_close_flag()
    {
        var suit_id = jQuery("#suit_file_id").val();
        ...
        jQuery.ajax({
            ...
            success: function(response){
                ...
                {
                    var acc_close_flag = json['close_flag'][0];
                    if(acc_close_flag!='Y')
                    {
                        jQuery("#final_remarks").jqxComboBox('val',1);
                    }
    ...
    function check_duplicate_bill(activities_id,lawyer,date)
    {
        var check = 1;
        var csrfName = jQuery('.txt_csrfname').attr('name'); // Value specified in $config['csrf_token_name']
        var csrfHash = jQuery('.txt_csrfname').val(); // CSRF hash
        ...
        jQuery.ajax({
            ...
            cache: false,
            async:false,
            url: "<?=base_url()?>legal_status_expense/check_duplicate_bill",
            data : {[csrfName]: csrfHash,activities_id:activities_id,lawyer:lawyer,date:date,suit_id:suit_id},
            datatype: "json",
            success: function(response){
            //alert(response);
                var json = jQuery.parseJSON(response);
    ...
    function make_default_block_field_list()
    {
        //'case_dt####input','case_sts####combo',
        //'next_case_dt####input','next_dt_sts####checkbox',
        //'next_date_purpose####combo',
        //'next_dt_remarks####input',
        block_field_list = [
            'district####combo',
            'plaintiff####combo','lawyer####combo',
            'court####combo',
            'remarks####input'
        ];
    }
    function get_information_by_district()
    {
        ...
        var csrfHash = jQuery('.txt_csrfname').val(); // CSRF hash
        jQuery.ajax({
        url: '<?=base_url()?>index.php/Home/get_information_by_legal_district',
        async:false,
        type: "post",
        ...
        success: function(response){
            ...
            jQuery.each(json['lawyer'],function(key,obj){
                change_lawyer.push({ value: obj.id, label: obj.name });
            });
            var plaintiff = [];
            jQuery.each(json['plaintiff'],function(key,obj){
                plaintiff.push({ value: obj.id, label: obj.name+'('+obj.pin+')' });
            });
            jQuery("#lawyer").jqxComboBox({theme: theme,  autoOpen: false,autoDropDownHeight: false, promptText: "Select Lawyer", source: change_lawyer, width: 250, height: 25});
    ...
    function details(id,operation,suit_file_id)
	{
		...
		jQuery('#hidden_suit_file_id').val(suit_file_id);
    ...
    function searchsuit()// customer search 
    {
        ...
        else
        {
            ...
            jQuery.ajax({
                ...
                data : {[csrfName]: csrfHash,search_dt:search_dt,case_number:case_number,proposed_type:proposed_type,req_type:req_type_id,loan_ac: loan_ac, case_number:case_number,hidden_loan_ac:hidden_loan_ac},
                datatype: "html",
                success: function(response){
    ...
    function edit(id,index,ho_edit_sts=null){
        ...
        jQuery.ajax({
            type: "POST",
            cache: false,
            url: "<?=base_url()?>legal_status_expense/get_edit_info",
            ...
            success: function(response){
                ...
                if(json.Message=='ok'){
                    ...
                    if(json.result.next_date_purpose!=0)
                    {
                    ...
                    {
                        //jQuery('#plaintiff').jqxComboBox('val', json.result.new_plaintiff+'_'+json.result.new_district);
                    ...
                    {
                        if (json.result.not_available_sts==1)
                        {
                    ...
                    var i=0;
                    if(ho_edit_sts!=1) //when not ho edit after verify
                    {
                    ...
                    jQuery.each(json['expense_data'],function(key,obj){
                        ...
                        html_string = '';
                        block_field_list.push("expense_type_"+new_counter+"####combo");
                        block_field_list.push("vendor_id_"+new_counter+"####combo");
                        //block_field_list.push("activities_name_"+new_counter+"####combo");
                            ...
                            html_string += '</td>';
                            html_string += '<td><div id="expense_type_'+new_counter+'" name="expense_type_'+new_counter+'" style="padding-left: 3px;margin-left:5px;" onchange="change_vendor(\'expense_type_'+new_counter+'\',1)"></div></td>';
                            ...
                            if(ho_edit_sts==1)
                            {
                                var ho_edit_str = 'disabled="disabled"';
                            }
                            else
                            {
                                var ho_edit_str = '';
                            ...
                            if(json.package_sts==1)//when package available
                            {
                                ...
                                {
                                    html_string +='<td style="text-align:center"><input type="checkbox" '+ho_edit_str+' name="chkBoxSelect'+new_counter+'" id="chkBoxSelect'+new_counter+'" onClick="CheckChanged_2(this,'+new_counter+')"/><input type="hidden" name="package_delete_'+new_counter+'" id="package_delete_'+new_counter+'" value="1"></td>';
                                }
                                else
                                {
                                    html_string +='<td style="text-align:center"><input type="checkbox" '+ho_edit_str+' checked name="chkBoxSelect'+new_counter+'" id="chkBoxSelect'+new_counter+'" onClick="CheckChanged_2(this,'+new_counter+')"/><input type="hidden" name="package_delete_'+new_counter+'" id="package_delete_'+new_counter+'" value="0"></td>';
                                }
                            }
                            else
                            {
                    ...
                    if(package_sts==1 && ho_edit_sts!=1)//when package available
                    {
                    ...
                    if(ho_edit_sts==1)
                    {
                        block_fields_for_any_time_edit(block_field_list);
    ...
    function load_filing_info(){
        ...
        jQuery.ajax({
            type: "POST",
            cache: false,
            url: "<?=base_url()?>legal_status_expense/get_add_info",
            data : {[csrfName]: csrfHash,id:val},
            datatype: "json",
            async:false,
            success: function(response){
                ...
                if(json.Message=='ok'){
                    ...
                    // jQuery('#lawyer').jqxComboBox('val', json.row_info.prest_lawyer_name);
                    // jQuery('#lawyer').jqxComboBox('val', json.row_info.prest_lawyer_name);
					...
					html +='<tr id="expense_1">';
							...
							html +='</td>';
							html +='<td>';
								html +='<div id="district_1" name="district_1" style="padding-left: 3px;margin-left:5px;display:none;width:90%" onchange="get_dropdown_data(1)"></div>';
								html +='<div id="vendor_id_1" name="vendor_id_1" style="padding-left: 3px;margin-left:5px;display:none;width:90%"></div>';
        						html +='<input name="vendor_name_1" type="text" class="" style="width:98%" id="vendor_name_1" />';
							html +='</td>';
							html +='<td><div id="activities_name_1" name="activities_name_1" style="padding-left: 3px;margin-left:5px;width:90%" onchange="get_expense_amount(\'activities_name_1\',1)"></div></td>';
							...
							html +='<td><input readonly name="amount_1" type="text" class="" style="width:90%" id="amount_1" /></td>';
							
                            if(json.package_sts==1)//when package available
                            {
                                html +='<td style="text-align:center"><input type="checkbox" name="chkBoxSelect1" id="chkBoxSelect1" checked onClick="CheckChanged_2(this,1)"/><input type="hidden" name="package_delete_1" id="package_delete_1" value="0"></td>';
                            }
                            else
                            {
                                html +='<td>&nbsp;<input type="hidden" name="package_delete_1" id="package_delete_1" value="1"></td>';
                            }
			        ...
			        datePicker_without_advance('activities_date_1');
			        jQuery('#expense_type_1').jqxComboBox('val', 1);
					jQuery("#vendor_id_1").jqxComboBox('val',json.row_info.prest_lawyer_name);
                    loan_segment = json.row_info.loan_segment;
                    make_expense_amount_editable('expense_counter',json.row_info.loan_segment);
                    ...
                    {
                        jQuery('#case_dt').val(next_dt);
                        //jQuery("#case_sts").jqxComboBox('val',next_sts);//present case status not set by the mail request from prince bhai (14-12-2022)
                        set_expense_date('case_dt');
                    }
                    else if(parseInt(dif)>0)//when next date is upcoming
                    {
                    ...
                    // This bellow condition will be work for some days as per prince bhai mail request
                    if(json.row_info.final_remarks=='2')
                    {
                        jQuery('#case_dt').val(json.row_info.prev_date);
                        jQuery("#case_sts").jqxComboBox('val',json.row_info.case_sts_prev_dt);
                        set_expense_date('case_dt');
                    }
                    var html='';
                    var size = Object.keys(json['status_history']).length;
    ...
    function clear_form() {
        ...
        jQuery("#hidden_suit_file_id").val('');
    ...
    function expense_validation()
	{
		...
		for (i=1;i<=counter;i++) 
		{
			if(jQuery('#expense_delete_'+i).val()==0) 
			{
                ...
                {
                    //when activities case withdraw by presenect case status should be case withdraw (mail request by prince 14-12-2022)
                    if((req_type == 2 && act.value==13) || (req_type != 2 && act.value==12))
                    {
                        jQuery("#case_sts").jqxComboBox('val',3);
                    }
                }
                ...
                var data = item2.value+'##'+jQuery('#activities_date_'+i).val()+'##'+act.value;
                ...
                if(act!=null && item2!=null)
                {
                    if(jQuery('#add_edit').val()=='add' || (jQuery('#add_edit').val()=='edit' && jQuery("#expense_edit_"+i).val()=='0'))
                    {
                        if(check_duplicate_bill(act.value,item2.value,jQuery('#activities_date_'+i).val())==0)
                        {
                            alert('Duplicate Bill');
                            jQuery('#activities_name_'+i+' input').focus();
                            return false;
                            ...
                            <div id='jqxTabs'>
                                               ...
                                               <div style="padding: 0px;width:100%;height:50px; border:1px solid #c0c0c0;font-family: Calibri;font-size: 14px">
                                                    <table id="deal_body" style="display:block;width:100%">
                                                        <tr>
                                                            ...
                                                            <td style="text-align:left;width:12%"><strong>Propsed Type&nbsp;&nbsp;</strong> </td>
                                        ...
                                        <div id="status_form_div" style="display:none">
                                            <div id="back_area" style="text-align: center;padding: 0px;width:100%;height:50px; border:1px solid #c0c0c0;font-family: Calibri;font-size: 14px">
                                              ...
                                              <div id="suit_form_area">
                                                <form method="POST" name="case_sts_form" id="case_sts_form"  style="margin:0px;" action="<?=base_url()?>index.php/legal_status_expense/add_edit" enctype="multipart/form-data">
                                                            ...
                                                            <tr>
                                                                <td width="50%">
																	<table style="width: 100%;">
																		...
																		<tr>
																			...
																			<td width="60%" ><input type="text" name="case_dt" tabindex="3" onchange="check_hiliday('case_dt'),set_expense_date('case_dt')" placeholder="dd/mm/yyyy" style="width:250px;" id="case_dt" value="" ><script type="text/javascript" charset="utf-8">datePicker_without_advance ("case_dt");</script></td>
																		</tr>	
                                                                        <tr>
                                                                            <td width="40%" style="font-weight: bold;">Present Case Status<span style="color:red">*</span> </td>
                                                                            <td width="60%" ><div id="case_sts" name="case_sts" style="padding-left: 3px" tabindex="6"></div></td>
                                                                        </tr>
                                                                        <tr>
                                                                        ...
                                                                        <tr id="optional_attachment_row" style="display:none">
                                                                            <td width="40%" style="font-weight: bold;"><span id="attachment_name"></span> Copy<span id="star" style="color:green">*</span> </td>
                                                                            <td width="60%" >
                                                                        ...
                                                                        <tr id="next_dt_row">
                                                                            ...
                                                                            <td width="60%" >
                                                                                <input type="text" name="next_case_dt" tabindex="4" onchange="check_hiliday('next_case_dt')" placeholder="dd/mm/yyyy" style="width:225px;" id="next_case_dt" value="" >
                                                                        ...
                                                                        <tr id="next_sts_row">
                                                                            <td width="40%" style="font-weight: bold;">Next Case Status<span style="color:green">*</span></td>
                                                                            <td width="60%" ><div id="next_date_purpose" name="next_date_purpose" style="padding-left: 3px" tabindex="5"></div></td>
                                                                        </tr>
                                                                        
                                                                        <tr id="next_remarks_row">
                                                                            <td width="40%" style="font-weight: bold;">Next Date Case Status Remarks</td>
                                                                            <td width="60%" ><textarea name="next_dt_remarks" tabindex="7" class="text-input-big" id="next_dt_remarks" style="height:40px !important;width:250px"></textarea></td>
                                                                        </tr>   
																	</table>
																</td>
																...
																<td width="50%">
																	<table style="width: 100%;">
                                                                        ...
                                                                        <tr>
                                                                            <td width="40%" style="font-weight: bold;">Transfer To Other District Plaintiff</td>
                                                                            <td width="60%" ><div id="plaintiff" name="plaintiff" style="padding-left: 3px" tabindex="8"></div></td>
                                                                        </tr>   
                                                                        <tr>
                                                                            <td width="40%" style="font-weight: bold;">New Lawyer Name</td>
                                                                            <td width="60%" ><div id="lawyer" name="lawyer" style="padding-left: 3px" tabindex="1"></div></td>
                                                                        </tr>
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <td width="60%" ><div id="court" name="court" style="padding-left: 3px;float:left" tabindex="2"></div><input type="button" style="float:left;margin-left:5px;margin-top:4px" title="Add More" onClick="add_more_ref_data('ref_court')" class="addmore"></td>
                                                                        </tr>
                                                                        <tr>
                                ...
                                <div style="overflow: hidden;">
                                    ...
                                    <div style="font-family: Calibri; margin: 0 0 -10px 0;font-size:14px;color:#0000cc">
                                        ...
                                        <strong>U = </strong>Next Date Update&nbsp;
                                    </div> <br/>
                                    </div>
                                </div>
                            </div>
...
<!-- Modal for product details -->
    ...
    <div style="padding-left: 17px"><strong><label id="header_title"></label></strong></div>
		<form method="POST" name="action_form" id="action_form"  style="margin:0px;">
    		...
    		<input name="hidden_suit_file_id" id="hidden_suit_file_id" value="" type="hidden">
...
var theme = getDemoTheme();
    rules= [
            ...
            { input: '#case_dt', message: 'above date from next date not allowed!', action: 'keyup, blur, change', rule: function(input,commit){
                if(jQuery("#next_case_dt").val()!='' && jQuery("#case_dt").val()!='' && jQuery("#next_dt_sts_value").val()==1)
                {
                    return date_validation(jQuery("#next_case_dt").val(),jQuery("#case_dt").val());
                }
                else
                {
                    return true;
            ...
            { input: '#case_dt', message: 'advance date not allowed!', action: 'keyup, blur, change', rule: function(input,commit){
                if(jQuery("#case_dt").val()!='')
                {
                    var today = new Date();
                    var arrDate = jQuery("#case_dt").val().split("/");
            ...
            { input: '#next_case_dt', message: 'required!', action: 'keyup, blur, change', rule: function(input,commit){
                var item = jQuery("#final_remarks").jqxComboBox('getSelectedItem');
                var item2 = jQuery("#case_sts").jqxComboBox('getSelectedItem');
                if(jQuery("#next_case_dt").val()=='' && jQuery("#next_dt_sts_value").val()==1 && item.value!=2 && item2!=null && item2.value!=15)
                {
                    return false;
                }
                else
            ...
            { input: '#next_case_dt', message: 'old date from case date not allowed!', action: 'keyup, blur, change', rule: function(input,commit){
                if(jQuery("#next_case_dt").val()!='' && jQuery("#case_dt").val()!='' && jQuery("#next_dt_sts_value").val()==1)
                {
                    return date_validation(jQuery("#next_case_dt").val(),jQuery("#case_dt").val());
                }
            ...
            { input: '#next_case_dt', message: 'next date and present date same!', action: 'keyup, blur, change', rule: function(input,commit){
                if(jQuery("#next_case_dt").val()!='' && jQuery("#case_dt").val()!='' && jQuery("#next_dt_sts_value").val()==1)
                {
                    if(jQuery("#next_case_dt").val()==jQuery("#case_dt").val())
                    {
                        return false;
                    }
                    else
    ...
    var options = { 
        ...
        jQuery("#sendButton").click( function() {
            ...
            var item = jQuery("#case_sts").jqxComboBox('getSelectedItem');
            if(item!=null && item.value==15 && req_type==99999999)
            {
                ...
                {
                    alert('Please Upload Judgement Copy');
                    return false;
                }
            }
                var validationResult = function (isValid) {
